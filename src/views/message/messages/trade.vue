<template>
  <base-page>
    ...
    <page-popup ref="pagePopup" position="right" class="pd-t44" style="background:#eee" type="msg">
      ...
      <MsgDetail :detail="detail" iconClass="message-icon2"/>
    </page-popup>
  </base-page>
</template>
...
export default {
  ...
  methods: {
    async getMessage() {
      ...
      let res = await this.$api.Message.tradeMessage(param);
      this.haveMsg = (res.tradeMessage || []).length > 0;
      this.items = res.tradeMessage
    }
  },
  async created() {
