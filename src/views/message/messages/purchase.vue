  ...
  <base-page>
    ...
    <ul class="message-wrapper" v-if="userInfo&&haveMsg">
      <li v-for="item in items" :key="item.id" class="flex bgfff border-beee align-middle pd5" @click="detail=item;$refs.pagePopup.open()">
        <div class="flex-none message-icon message-icon3 flex-none"/>
    ...
    <page-popup ref="pagePopup" position="right" class="pd-t44" style="background:#eee" type="msg">
      ...
      <MsgDetail :detail="detail" iconClass="message-icon3"/>
    </page-popup>
  </base-page>
</template>
...
export default {
  ...
  methods: {
    async getMessage() {
      ...
      let res = await this.$api.Message.purchaseMessage(param);
      this.haveMsg = (res.purchaseMessage || []).length > 0;
      this.items = res.purchaseMessage
    }
  },
  async created() {
